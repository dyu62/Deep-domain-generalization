...
def triplets(edge_index, num_nodes):
    ...
    idx_k = adj_t_col.storage.row()
    edx_2nd = adj_t_col.storage.value()
    mask1 = (idx_i == idx_k) & (idx_j != idx_i)
    mask2 = (idx_i == idx_j) & (idx_j != idx_k)
    mask3 = (idx_j == idx_k) & (idx_i != idx_k)
    mask4 = (idx_j == idx_k) & (idx_i == idx_k)
    # mask = ~(mask1 | mask2 | mask3)
    mask = ~(mask1 | mask2 | mask3 | mask4)
    idx_i, idx_j, idx_k, edx_1st, edx_2nd = idx_i[mask], idx_j[
        mask], idx_k[mask], edx_1st[mask], edx_2nd[mask]
    num_triplets_real = torch.cumsum(
        num_triplets, dim=0) - torch.cumsum(~mask, dim=0)[torch.cumsum(num_triplets, dim=0)-1]
